...
sub _processFinishedBuild {
    ...
    if ($backup_build_deps) {
        ...
        foreach my $initial_path (@initial_paths) {
            my $drv_path = $MACHINE_LOCAL_STORE->queryDeriver($initial_path);
            chomp $drv_path;
            next if $drv_path eq '';
    ...
    foreach my $path (@needed_paths) {
        ...
        next if ($path =~ /nixos-/ and $path =~ /loongarch64-linux\.iso/);
        ...
        if (system("zstd", "-z", "$tempdir/nar", "-o", "$tempdir/nar.zst") != 0) {
            print STDERR "S3Backup: Failed to compress NAR for $path\n";
            die;
        }
        ...
        unless (-f $nar_dest) {
            if (system("mv", "$tempdir/nar.zst", $nar_dest) != 0) {
                print STDERR "S3Backup: Failed to move compressed NAR file to publish directory: $nar_dest\n";
                die;
            }
        }
